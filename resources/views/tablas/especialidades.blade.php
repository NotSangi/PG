...
{!!Form::open(array('url' => 'updateEstadoEspecialidad', 'method' => 'POST', 'autocomplete' => 'off'))!!}
{{Form::token()}}
<thead>
    <tr>
        <th>ID</th>
...
<tbody>
    @foreach($especialidades  as $espe)
        <tr>
            <td>{{ $espe->id }}</td>
            <td>{{ $espe->name }}</td>
            <td>{{ $espe->description }}</td>
            <td>
                <select name="estado[{{ $espe->id }}]"
                    style="width: 100%; border: 0.5px solid grey; border-radius: 0.5rem; color: grey;">
                    <?php    if (($espe->estado) == 1) { ?>
                    <option selected disabled value="{{$espe->estado}}">ACTIVO</option>
                    <?php    } else { ?>
                    <option selected disabled value="{{$espe->estado}}">INACTIVO</option>
                    <?php    } ?>
...
{!!Form::open(array('url' => 'crearEspecialidad', 'method' => 'POST', 'autocomplete' => 'off'))!!}
{{Form::token()}}
<div class="modal fade" id="modalCrear" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="modalCrearLabel">Especialidad</h1>
